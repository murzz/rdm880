...
namespace rdm
{
   namespace message
   {
      ...
      namespace framing
      {
         ...
         data_type::value_type calculate_bcc(const data_type & packet,
               data_type::size_type start_idx = 0,
         ...
         bool verify(const data_type & frame)
         {
            ...
            // comparing lengths
            const data_type::size_type expected_frame_size =
                  size::stx + size::device_addr + size::data_len +
                        data_len +
                        size::bcc + size::etx;
